...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
    ...
    try {
      ...
      if (!event.forceRefresh && _cachedProfile != null && _lastFetchTime != null) {
        ...
        if (cacheDuration.inSeconds < _cacheDurationSeconds) {
          _logger.d('Using cached profile data (cache age: ${cacheDuration.inSeconds}s)');
      ...
      if (userRepository != null) {
        try {
          ...
          if (user.rawProfileData != null) {
            _logger.d('Using rawProfileData to create UserProfile');
            _logger.d('Raw profile data: ${user.rawProfileData}');
            // Create UserProfile from the raw data
            final userProfile = UserProfile.fromJson(user.rawProfileData!);
            _logger.d('Created UserProfile: ${userProfile.basicInfo.fullName}, transactions: ${userProfile.transactionStats.totalTransactions}');
          ...
          _logger.d('Converting User model to UserProfile');
          final userProfile = UserProfile.fromUserModel(user);
          _logger.d('Converted to UserProfile: ${userProfile.basicInfo.fullName}, transactions: ${userProfile.transactionStats.totalTransactions}');
          _updateCache(userProfile);
          emit(ProfileLoaded(userProfile: userProfile));
          return;
        } catch (repoError) {
          // If using userRepository fails, fall back to the direct API call
          _logger.e('UserRepository error: $repoError. Falling back to direct API call.');
        }
      }

      // Fall back to original implementation using AuthService
      _logger.d('Using AuthService to get profile data');
    ...
    try {
      ...
      if (userRepository != null) {
        try {
          ...
          _logger.e('UserRepository update error: $repoError. Falling back to direct API call.');
        }
      }
  ...
  void _updateCache(UserProfile profile) {
    ...
    _logger.d('Updated profile cache');
  }
}