...
import '../core/services/logger_service.dart'; // Thêm import cho LoggerService
...
class AuthService {
  ...
  Future<bool> isAdmin() async {
    ...
    if (_cachedAdminStatus != null && _cachedAdminStatusTime != null) {
      ...
      if (cacheDuration.inSeconds < 10) {
        LoggerService.debug('===== ADMIN CHECK =====');
        LoggerService.debug('Using cached admin status: $_cachedAdminStatus (cache age: ${cacheDuration.inSeconds}s)');
        LoggerService.debug('=====================');
    ...
    try {
      ...
      LoggerService.debug('===== ADMIN CHECK =====');
      LoggerService.debug('Checking admin status with token: ${token != null ? token.substring(0, Math.min(20, token.length)) : "null"}...');

      if (token == null) {
        LoggerService.debug('ADMIN CHECK RESULT: Token is null, user is NOT ADMIN');
      ...
      if (parts.length != 3) {
        LoggerService.debug('ADMIN CHECK RESULT: Invalid token format, parts length: ${parts.length}, user is NOT ADMIN');
        _updateAdminCache(false);
        return false;
      }
      ...
      try {
        ...
        LoggerService.debug('JWT payload: $payloadMap');
        ...
        LoggerService.debug('ADMIN CHECK RESULT: Token does not contain roles or roles is not a list, user is NOT ADMIN');
        _updateAdminCache(false);
        return false;
      } catch (e) {
        LoggerService.debug('ADMIN CHECK RESULT: Error parsing token payload: $e, user is NOT ADMIN');
        _updateAdminCache(false);
        return false;
      }
    } catch (e) {
      LoggerService.error('ADMIN CHECK RESULT: Error in isAdmin()', e);
      _updateAdminCache(false);
      return false;
    } finally {
      LoggerService.debug('=====================');
  ...
  void _updateAdminCache(bool status) {
    ...
    LoggerService.debug('Updated admin status cache: $status');
  }
  ...
  Future<bool> forceAdminCheck() async {
    ...
    LoggerService.debug('Force admin check - ignoring cache');
