...
class RewardService {
  ...
  Future<Map<String, dynamic>> getMyRewards({
    ...
    if (response.statusCode == 200) {
      ...
      AppLogger.d('Reward', 'Nhận ${(data['rewards'] as List).length} phần thưởng');
      if (fromDate != null || toDate != null) {
        AppLogger.d('Reward', 'Lọc theo ngày · từ $fromDate đến $toDate');
        ...
        if ((data['rewards'] as List).isNotEmpty) {
          ...
          AppLogger.d('Reward', 'Phần thưởng cũ nhất ${firstReward.earnedDate}, mới nhất ${lastReward.earnedDate}');
        }
      }
      ...
      AppLogger.w('Reward', 'x ${response.statusCode} GET /rewards/my-rewards · ${AppLogger.preview(response.body)}');
